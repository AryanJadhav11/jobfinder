...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['sendmail'])) {
    ...
    VALUES ('$userid', '$company_name', '$title', '$firstname', '$lastname', '$email', '$address', '$qualification', '$techskill', '$resume', '$city', '$zip', '$gender', '$phone_no', '$resumelink')";
    ...
    if ($coni->query($sql) === TRUE) {
      ...
      function smtp_mailer($to, $subject, $message, $resume) {
        ...
        $message .= '<ul><li><strong>First Name:</strong> ' . $firstname . '</li>';
...
<input type="hidden" class="form-control" id="comp_title" name="comp_title" value="<?php echo $tit; ?>" readonly>
